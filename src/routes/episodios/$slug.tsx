  ...
  return (
    <>
      ...
      <Section label="ROTEIRO" title="O que atravessamos">
        <ol className="grid gap-3 md:grid-cols-2">
          {episode.topics.map((topic, index) => (
            <li key={topic} className="border-b border-dashed py-4">
              <span className="label-mono mr-4 text-primary">0{index + 1}</span>
              {topic}
            </li>
          ))}
        </ol>
      ...
      <Section label="CAMADAS RELACIONADAS" title="A lei e os tribunais">
        ...
        <Link
          to="/episodios"
          className="label-mono mt-8 inline-block border-b border-primary pb-1 text-primary"
        >
